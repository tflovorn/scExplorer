...
func D1F0System(env *tempAll.Environment) (solve.DiffSystem, []float64) {
	...
	system := solve.Combine([]solve.Diffable{diffD1, diffF0})
	start := []float64{env.D1, env.F0}
	return system, start
}
...
func D1MuF0Solve(env *tempAll.Environment, epsAbs, epsRel float64) (vec.Vector, error) {
	...
	if env.A == 0.0 && env.B == 0.0 {
		...
		//D1, Mu_h, F0, Beta := env.D1, env.Mu_h, env.F0, env.Beta // cache env
		...
		fmt.Printf("got a=%f, b=%f at Tc\n", omegaFit[0], omegaFit[2])
		env.A, env.B = omegaFit[0], omegaFit[2]
		env.PairCoeffsReady = true
		// we are at T < T_c; uncache env
